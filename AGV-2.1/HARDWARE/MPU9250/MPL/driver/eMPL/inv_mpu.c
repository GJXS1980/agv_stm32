...
void delay_ms_os(u32 ms)
{
	delay_us(ms*1000);
}
#define i2c_write   MPU_Write_Len
#define i2c_read    MPU_Read_Len
#define delay_ms    delay_ms_os
...
u8 mpu_mpl_get_data(float *pitch,float *roll,float *yaw)
{
    ...
    int8_t accuracy;
	  ...
	  if(sensors&INV_WXYZ_QUAT) 
	  {
			 Send_Data.quat[0] = quat[0] / q30;	//q30格式转换为浮点数
			 Send_Data.quat[1] = quat[1] / q30;
			 Send_Data.quat[2] = quat[2] / q30;
			 Send_Data.quat[3] = quat[3] / q30; 
	  }
    ...
    *yaw   =  data[2] / q16;		
		...
		my_accel[0]=accel_short[0];
		my_accel[1]=accel_short[1];
		my_accel[2]=accel_short[2];		
		
		my_magnet[0]=compass_short[0];
		my_magnet[1]=compass_short[1];
		my_magnet[2]=compass_short[2];		
